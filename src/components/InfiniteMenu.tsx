...
class IcosahedronGeometry extends Geometry {
  constructor(divisions = 0, radius = 1) {
    super();
...
class ArcballControl {
  ...
  pointerPos: vec2 = vec2.create();
  previousPointerPos: vec2 = vec2.create();
  _rotationVelocity: number = 0; // smooth rotational velocity
  _combinedQuat: quat = quat.create();
...
// ... (Keeping your existing shader sources: discVertShaderSource, discFragShaderSource unchanged)
// ... (Keeping your existing Geometry, Face, Vertex, IcosahedronGeometry, DiscGeometry classes unchanged)
// ... (Keeping your existing WebGL utility functions: createShader, createProgram, etc. unchanged)
// ... (Keeping your existing ArcballControl class unchanged)
...
class InfiniteGridMenu {
  ...
  discBuffers: { vertices: Float32Array; indices: Uint16Array; uvs: Float32Array; normals: Float32Array };
  ...
  discLocations: {
    ...
    uScaleFactor: WebGLUniformLocation;
    uRotationAxisVelocity: WebGLUniformLocation;
    uTex: WebGLUniformLocation;
    uFrames: WebGLUniformLocation;
    uItemCount: WebGLUniformLocation;
    uAtlasSize: WebGLUniformLocation;
  };
  ...
  onItemClick: (index: number) => void; // Added for click handling
  drawBufferSize: vec2;
  worldMatrix: mat4;
  ...
  tex: any;
  atlasSize: number;
  discInstances: { matricesArray: Float32Array<ArrayBuffer>; matrices: any[]; buffer: any; };
  ...
  constructor(
    ...
    onMovementChange,
    onInit = null,
    onItemClick = null // Added to constructor
  ) {
    this.canvas = canvas;
    this.items = items || [];
    this.onActiveItemChange = onActiveItemChange || (() => {});
    this.onMovementChange = onMovementChange || (() => {});
    this.onItemClick = onItemClick || (() => {}); // Initialize callback
    this.#init(onInit);
  }
  ...
  #init(onInit) {
    this.gl = this.canvas.getContext('webgl2', { antialias: true, alpha: true }); // Changed alpha to true for transparency
    const gl = this.gl;
    if (!gl) {
  ...
  #handleClick(event) {
    ...
    // Convert to WebGL coordinates (0 to 1, flip y)
    const normalizedX = (x / rect.width) * 2 - 1;
    const normalizedY = -((y / rect.height) * 2 - 1);

    // Simple picking: Find the nearest disc in front (z-depth check)
    ...
    positions.forEach((pos, index) => {
      const screenPos = vec4.transformMat4(
        vec4.create(),
        vec4.fromValues(pos[0], pos[1], pos[2], 1),
        mat4.multiply(mat4.create(), this.camera.matrices.projection, this.camera.matrices.view)
      );
      const ndc = vec3.fromValues(screenPos[0] / screenPos[3], screenPos[1] / screenPos[3], screenPos[2] / screenPos[3]);

      const dist = vec2.distance(vec2.fromValues(normalizedX, normalizedY), vec2.fromValues(ndc[0], ndc[1]));
      if (dist < 0.1 && ndc[2] < minZ) { // Adjust 0.1 for sensitivity
        minZ = ndc[2];
        clickedIndex = index % this.items.length;
      }
    });

    if (clickedIndex !== -1) {
      this.onItemClick(clickedIndex); // Trigger the callback
    }
  }
...
const defaultItems = [
  ...
  { title: 'Default Item 1', description: 'Description for default item 1', image: 'path/to/image1.jpg', link: 'http://example.com' },
  { title: 'Default Item 2', description: 'Description for default item 2', image: 'path/to/image2.jpg', link: 'http://example.com' },
];
...
export default function InfiniteMenu({ items = [] }) {
  ...
  useEffect(() => {
    ...
    const handleItemClick = (index) => {
      setSelectedImage(items[index].image); // Set image for popup
    };
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(
        ...
        setIsMoving,
        (sk) => sk.run(),
        handleItemClick // Pass click handler
      );
    }
